...
namespace DAL.Base.EF.Repositories
{
        ...
        where TDbContext : DbContext
    {
        ...
        {
            return (await RepoDbSet.AsNoTracking().ToListAsync()).Select(domainEntity =>
                Mapper.MapDomainToDAL(domainEntity));
        }
        ...
        {
            return Mapper.MapDomainToDAL(RepoDbSet.Add(Mapper.MapDALToDomain(entity)).Entity);
        }

        public virtual TDALEntity Update(TDALEntity entity)
        {
            return Mapper.MapDomainToDAL(RepoDbSet.Update(Mapper.MapDALToDomain(entity)).Entity);
        }

        public virtual TDALEntity Remove(TDALEntity entity)
        {
            return Mapper.MapDomainToDAL(RepoDbSet.Remove(Mapper.MapDALToDomain(entity)).Entity);
        }

        public virtual async Task<TDALEntity> RemoveAsync(TKey id)
        {
            return Mapper.MapDomainToDAL(RepoDbSet.Remove(await RepoDbSet.FindAsync(id)).Entity);
        }
    }
}